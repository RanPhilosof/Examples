...
using Elisra.Infra.FullDuplexMultiClientCommunication;
...
public class TcpServerApp : IServerCommunication
{
    private ByteArray byteArray;
    private Elisra.Infra.FullDuplexMultiClientCommunication.Server<ByteArray> server;
    ...
    public TcpServerApp(int port, int keepAliveTimeout, int bufferSize, int maxClients)
    {
        byteArray = new ByteArray(bufferSize);

        var byteArrayMessageCreator = new ByteArrayMessageCreator(bufferSize);

        server = new Elisra.Infra.FullDuplexMultiClientCommunication.Server<ByteArray>(port.ToString(), 0, 0, bufferSize, bufferSize, byteArrayMessageCreator);
...
public class TcpClientApp : IClientCommunication
{
    private ByteArray byteArray;

    private Elisra.Infra.FullDuplexMultiClientCommunication.Client<ByteArray> client;
    ...
    public TcpClientApp(string serverAddress, int serverPort, int bufferSize)
    {
        byteArray = new ByteArray(bufferSize);

        var byteArrayMessageCreator = new ByteArrayMessageCreator(bufferSize);
        client = new Elisra.Infra.FullDuplexMultiClientCommunication.TcpClient<ByteArray>(
            serverAddress,
            serverPort.ToString(),
